...
public class q1server{
	...
	Socket client;
	//To create a new server socket on port 9999
	int port = 9999;

	//For sending messages to the client
	PrintWriter output_to_client;

	//To get input messages from client
	...
	public void setup(){
		...
		try{
			System.out.println("Starting TCP Server on Port 9999");
			//Setup a new server socket on port:9999
			server = new ServerSocket(port);
			//Accept Client socket to establish connection
	...
	public void execute(){
		try{
			...
			while(true){
				...
				message_from_client = input_from_client.readLine();
				//Check if the message contains the diffie-hellman Key Exchange
				if (message_from_client.contains("Diffie-Hellman Key Exchange:")){
					System.out.println("Diffie-Hellman Key Exchange ");
					//Split message into parts divided by :
					String[] split_message = message_from_client.split(":");
					//Part of the message contains the computed A = base^a mod p
					A = Integer.parseInt(split_message[1]);
					
					//Calculate the shared secret key as part of the Diffie Hellman Algorithm
					shared_secret = (A^b) % prime;
					System.out.println("Shared Secret Value: " + shared_secret);

					//B = base^b mod p
					B = (base^b)%prime;
					//Send Calculated B to client as part of Diffie Hellman Key Exchange
					output_to_client.println("Key Exchange B:" + B);
					output_to_client.flush();
				}
				else{
					//Get a message from the client
					System.out.println("Received Encrypted Message: " + message_from_client);
					//Decrypt the message
					//First decrypt using transposition matrix scheme
					String decrypted_message = decrypt(message_from_client,shared_secret);
					...
					//encrypt the message to send back
					//First encrypt using substitution scheme
					String encrypted_message = sub_encrypt(decrypted_message,shared_secret);
					//Then encrypt using transposition matrix scheme
					encrypted_message = encrypt(encrypted_message,shared_secret);
					System.out.println("Sending back Encrypted Message as ACK: "+ encrypted_message);

					//Send message to client
					output_to_client.println(encrypted_message);
					output_to_client.flush();
				}
			}
		}
	...
	/************************************************************************/
	//Get the shared secret key from the value of A computed by the client
